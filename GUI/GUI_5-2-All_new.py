...
def intro():
    ...
    global intesnsities, pixel_counts, intesnsities_R, pixel_counts_R, intesnsities_L, pixel_counts_L, means, means_L, means_R
...
# model.load_weights(MODEL_DIR + r"\lug segmentation20211010T1809\unet_r231covid-0de78a7e.pth", by_name=True)
dicts = torch.load(MODEL_DIR + r'\lug segmentation20211010T1809\unet_r231covid-0de78a7e.pth',
...
def single_predict(xcord=450, ycord=100):
    ...
    if not os.path.exists(Directory+"/corr_predicts/" + Name + ".png") and not os.path.exists(
            load_directory + "/corr_predicts/" + Name + ".png"):

        if risk_scores[all_names.index(Name)] == "NI":
            ...
            center_view()
            return
        if risk_scores[all_names.index(Name)] == "NL":
        ...
        if len(np.unique(total_lung)) > 1:
            ...
            left_scores[original_all_names.index(Name)] = float(
                np.count_nonzero(left_infection) / (np.count_nonzero(left_lung) + 1)) * 100
            right_scores[original_all_names.index(Name)] = float(
                np.count_nonzero(right_infection) / (np.count_nonzero(right_lung) + 1)) * 100
            risk_scores[original_all_names.index(Name)] = float(
                np.count_nonzero(total_infection) / (np.count_nonzero(total_lung) + 1)) * 100
            ...
            means[original_all_names.index(Name)] = intesnsities[original_all_names.index(Name)] / (
                        pixel_counts[original_all_names.index(Name)] + 1)
            means_L[original_all_names.index(Name)] = intesnsities_L[original_all_names.index(Name)] / (
                        pixel_counts_L[original_all_names.index(Name)] + 1)
            means_R[original_all_names.index(Name)] = intesnsities_R[original_all_names.index(Name)] / (
                        pixel_counts_R[original_all_names.index(Name)] + 1)
            ...
            if len(np.unique(total_infection)) > 1:
                ...
                plt.imsave('predict.png', total_infection_p, cmap=CMAP)
                img1 = cv.imread(Directory+"/CT_images/" + Name + ".png")
                ...
                # Label(root, text= "No infection found.                  ", foreground='red').place(x=1350, y=100)
                status_single.config(text="No infection found.", foreground='red')
                risk_scores[all_names.index(Name)] = "NI"
                predict_toggle = False
                return
            ...
            # Label(root, text= "No lung found.                  ", foreground='red').place(x=1350, y=100)
            status_single.config(text="No lung found.", foreground='red')
            risk_scores[all_names.index(Name)] = "NL"
            predict_toggle = False
            return
    ...
    if not load_toggle:
        density_score = np.sum(intesnsities) / np.sum(pixel_counts)
        left_density_score = np.sum(intesnsities_L) / np.sum(pixel_counts_L)
        right_density_score = np.sum(intesnsities_R) / np.sum(pixel_counts_R)
...
def predict_all():
    ...
    global accuracy_scores, intesnsities, intesnsities_L, intesnsities_R, pixel_counts, pixel_counts_L, pixel_counts_R
    ...
    for N in original_all_names:
        ...
        if len(np.unique(total_lung)) > 1:
            ...
            left_scores[original_all_names.index(N)] = float(
                np.count_nonzero(left_infection) / (np.count_nonzero(left_lung) + 1)) * 100
            right_scores[original_all_names.index(N)] = float(
                np.count_nonzero(right_infection) / (np.count_nonzero(right_lung) + 1)) * 100
            risk_scores[original_all_names.index(N)] = float(
                np.count_nonzero(total_infection) / (np.count_nonzero(total_lung) + 1)) * 100
            ...
            means[original_all_names.index(N)] = intesnsities[original_all_names.index(N)] / (
                        pixel_counts[original_all_names.index(N)] + 1)
            means_L[original_all_names.index(N)] = intesnsities_L[original_all_names.index(N)] / (
                        pixel_counts_L[original_all_names.index(N)] + 1)
            means_R[original_all_names.index(N)] = intesnsities_R[original_all_names.index(N)] / (
                        pixel_counts_R[original_all_names.index(N)] + 1)
            ...
            if len(np.unique(total_infection)) > 1:
                ...
                risk_scores[all_names.index(N)] = "NI"

        else:
            risk_scores[all_names.index(N)] = "NL"
...
def L_lung_view():
    ...
    if os.path.exists(Directory+"/lung/" + Name + "_blended" ".png") or os.path.exists(
            load_directory + "/lung/" + Name + "_blended" ".png"):
...
def B_directory():
    ...
    # cleanfolder("CT_images")
    # cleanfolder("predictions")
    # cleanfolder("corr_predicts")
    # cleanfolder("lung")
    predict_toggle = False
    all_predict_toggle = False
    load_toggle = False
...
def risk_info():
    ...
    global ratings, std_T, std_L, std_R
    global total_score_conso, patient_left_score_conso, patient_right_score_conso, total_score_ggo, patient_left_score_ggo, patient_right_score_ggo
    ...
    Label(risk_frame, text=f"{left_scores[original_all_names.index(Name)]: .1f}" + " % ", background='white').grid(
        row=1, column=0)
    Label(risk_frame, text=f"{right_scores[original_all_names.index(Name)]: .1f}" + " % ", background='white').grid(
        row=1, column=1)
    ...
    try:
        Label(risk_frame, text=f"{risk_scores[original_all_names.index(Name)]: .1f}" + " % ", background='white').grid(
    ...
    Label(risk_frame, text="Segmentation Accuracy").grid(row=6, column=0, columnspan=2, pady=5)

    accuracy = tk.StringVar()
    accuracy_options = ttk.Combobox(risk_frame, width=27, textvariable=accuracy)
    ratings = ["No rating", "Poor", "Acceptable", "Good", "Excelent"]
    accuracy_options["values"] = ("No rating", "Poor", "Acceptable", "Good", "Excelent")
    ...
    accuracy_label.grid(row=8, column=0, columnspan=2, pady=5)
    ...
    if not load_toggle:
        try:
            total_score = (patient_infection / patient_lung) * 100
        except:
            total_score = float(0)

        try:
            patient_left_score = (patient_left_infection / patient_left_lung) * 100
        except:
            patient_left_score = float(0)

        try:
            patient_right_score = (patient_right_infection / patient_right_lung) * 100
        except:
            patient_right_score = float(0)

    if not load_toggle:
        try:
            total_score_conso = (patient_infection_conso / patient_lung) * 100
        except:
            total_score_conso = float(0)

        try:
            patient_left_score_conso = (patient_left_infection_conso / patient_left_lung) * 100
        except:
            patient_left_score_conso = float(0)

        try:
            patient_right_score_conso = (patient_right_infection_conso / patient_right_lung) * 100
        except:
            patient_right_score_conso = float(0)
    ...
    Label(risk_frame, text=f"    {total_score: .1f} %     Density(HU) : {int(density_score)}    ",
          background='white').grid(row=11, column=0, columnspan=2)
    Label(risk_frame, text=f" GGO   {total_score_ggo: .1f} %  ", background='yellow').grid(row=12, column=0,
                                                                                           columnspan=2)
    Label(risk_frame, text=f" CONSO   {total_score_conso: .1f} %  ", background='orange').grid(row=13, column=0,
                                                                                               columnspan=2)
    ...
    Label(risk_frame, text=f"    {patient_left_score: .1f} %     Density(HU) : {int(left_density_score)}    ",
          background='white').grid(row=17, column=0)
    Label(risk_frame, text=f"    {patient_right_score: .1f} %     Density(HU) : {int(right_density_score)}    ",
          background='white').grid(row=17, column=1)
...
def write_excel():
    global Name, patient_name, patient_left_score, patient_right_score, total_score
    ...
    global patient_left_score_ggo, patient_right_score_ggo, total_score_ggo, operator, Directory
    ...
    worksheet.write(2, 0, str(date.today()))
    worksheet.write(0, 1, 'Left score')
    worksheet.write(0, 2, 'Right score')
    worksheet.write(0, 3, 'Total score')
    worksheet.write(0, 4, 'Rating')
    worksheet.write(0, 5, 'Slice')
    worksheet.write(0, 7, 'patient_left_score')
    worksheet.write(1, 7, patient_left_score)
    worksheet.write(0, 8, 'patient_right_score')
    worksheet.write(1, 8, patient_right_score)
    worksheet.write(0, 9, 'total_score')
    worksheet.write(1, 9, total_score)
    ...
    worksheet.write(0, 13, 'Left_density')
    worksheet.write(1, 13, left_density_score)
    worksheet.write(0, 14, 'Right_density')
    worksheet.write(1, 14, right_density_score)
    worksheet.write(0, 15, 'Density')
    worksheet.write(1, 15, density_score)
    worksheet.write(0, 16, 'Left_std')
    worksheet.write(1, 16, std_L)
    worksheet.write(0, 17, 'Right_std')
    worksheet.write(1, 17, std_R)
    worksheet.write(0, 18, 'std')
    worksheet.write(1, 18, std_T)
...
def submit_scores():
    ...
    tk.Label(manual_frame, text="Submitted!", foreground='green').grid(row=6, column=0, columnspan=2)


def manual_assessment():
    global left_entry, right_entry, whole_entry

    Label(manual_frame, text="Patient Total Infection in Percentage").grid(row=0, column=0, columnspan=2)
    Label(manual_frame, text="Left Lung").grid(row=2, column=0, padx=65);
    tk.Label(manual_frame, text="Right Lung").grid(row=2, column=1, padx=65)

    whole_entry = tk.Entry(manual_frame)
    whole_entry.grid(row=1, column=0, padx=10, columnspan=2, pady=15)
    ...
    Label(manual_frame, text="   ").grid(row=4, column=0, columnspan=2)
    Button(manual_frame, text="Submit Evaluation", command=submit_scores).grid(row=5, column=0, columnspan=2)


def operator_name():
    global operator
    operator = name_entry.get()
    tk.Label(root, text=operator + ' ' * 20, foreground='green').place(x=1510, y=1020)
...
def excel_reader(dir):
    ...
    global left_density_score, right_density_score, density_score, std_T, std_L, std_R
    global patient_left_score_conso, patient_right_score_conso, total_score_conso
    global patient_left_score_ggo, patient_right_score_ggo, total_score_ggo, operator
...
Label(root, text="Risk Score", foreground='green').place(x=1550, y=50)
...
Label(root, text="Manual Assessment", foreground='green').place(x=1520, y=730)
manual_frame = tk.Frame(root, height=200, width=380, relief="groove", borderwidth=5, pady=10)
